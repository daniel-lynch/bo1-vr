 ...
 * *.asi and LoadLibrary()s each one from DllMain. vrsubmit.asi spawns a worker
 * thread (it must not work under the loader lock) and signals
 * "bo1vr_exp05_done" when finished; we wait on that.
 *
 * Identical to experiments/04_live_fntable/asihost.c except for the event name
 * and the longer timeout -- the submit loop takes as long as BO1VR_FRAMES says. */
#include <windows.h>
#include <stdio.h>
...
int main(void)
{
    ...
    w = WaitForSingleObject(done, 300000);
