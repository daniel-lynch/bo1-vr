# Toolchain notes and configuration decisions

> This document was the repository README while the toolchain foundation was
> being built (experiments 0–6, before anything hooked the real game). Its
> status tables describe that era; the project has since moved well past it —
> see the top-level [README](../README.md) for the current state. Everything
> else here — the build decisions and their reasons — still stands and is the
> reference for why the toolchain is configured the way it is.

The toolchain foundation: a working 32-bit `dinput8.dll` ASI loader and seven
verification experiments. At this stage nothing hooked the game yet — but the
VR chain itself was no longer hypothetical. A 32-bit
